...
int decrpyt(char *filename, char *filepath) {
	if (strstr(filename, ".csshwp") || strstr(filename, ".cssppt") || strstr(filename, ".csspptx") || strstr(filename, ".cssdoc") || strstr(filename, ".cssdocx") || strstr(filename, ".cssxls") || strstr(filename, ".cssxlsx")) {
		...
		sprintf_s(n_name, sizeof(n_name), "%s.%s", filefullname, identifier);
...
void delete_file()
{
	...
	char* thisFile = "training_decrypt.exe";
	char szBatFile[256];
